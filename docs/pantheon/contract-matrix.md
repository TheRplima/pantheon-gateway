...
| `agent.runtime.registered`         | Runtime worker           | API worker               | config persisted                       | `registering -> registered`            | persist runtime revision                                                           | `agent.activation.requested`                 |
| `agent.activation.requested`       | API worker               | Runtime/Workspace worker | desired state `active`                 | `registered OR inactive -> activating` | move disabled->active if needed, reload/debounce gateway                           | `agent.activated` or `agent.failed`          |
...
| `agent.deactivation.requested`     | API worker               | Runtime/Workspace worker | agent currently active or registered   | `active OR registered -> deactivating` | remove routing/registration, drain, move active->disabled                          | `agent.deactivated` or `agent.failed`        |
